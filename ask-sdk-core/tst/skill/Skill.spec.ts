...
describe('Skill', () => {
    ...
    it('should populate the ServiceClientFactory when an apiClient is provided', () => {
        ...
        return skill.invoke(requestEnvelope)
            .then(() => {
                const handlerInput = <HandlerInput> handler.handle.getCall(0).args[0];
                const factory = <any> handlerInput.serviceClientFactory;

                expect(handlerInput.serviceClientFactory).to.not.equal(undefined);
    ...
    it('should set ServiceClientFactory as undefined when the apiClient is missing', () => {
        ...
        return skill.invoke(requestEnvelope)
            .then(() => {
                const handlerInput = <HandlerInput> handler.handle.getCall(0).args[0];
                expect(handlerInput.serviceClientFactory).equal(undefined);
            });
    });
    ...
    it('should throw an error if skill id verification failed', async() => {
        ...
        try {
            ...
            expect(err.name).equal('AskSdk.Skill Error');
            expect(err.message).equal('Skill ID verification failed!');
