...
describe('DefaultErrorMapper', () => {
    it('should be able to get the ErrorHandler that can handle the error', async() => {
        ...
        const handler =  await mapper.getErrorHandler(null, null);

        expect(handler).instanceof(MockAlwaysTrueErrorHandler);
    });

    it('should return null if no ErrorHandler can handle the error', async() => {
        const mapper = new DefaultErrorMapper({
            errorHandlers : [
                new MockAlwaysFalseErrorHandler(),
            ],
