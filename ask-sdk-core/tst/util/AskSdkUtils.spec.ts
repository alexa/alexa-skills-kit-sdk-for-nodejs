...
describe('AskSdkUtils', () => {
    ...
    it('should be able to create user agent string', () => {
        const userAgent = createAskSdkUserAgent(undefined);
        const packageInfo = require('../../package.json');

        expect(userAgent).equal(`ask-node/${packageInfo.version} Node/${process.version}`);
    });

    it('should be able to create user agent string with custom user agent', () => {
        const userAgent = createAskSdkUserAgent('custom user agent');
        const packageInfo = require('../../package.json');

        expect(userAgent).equal(`ask-node/${packageInfo.version} Node/${process.version} custom user agent`);
    });
});
