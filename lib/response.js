...
module.exports = (function () {
    return {
        ...
        ':tellWithDisplayRenderTemplate': function(speechOutput, template) {
            if(this.isOverridden()) {
                return;
            }
            this.handler.response = buildSpeechletResponse({
                sessionAttributes: this.attributes,
                output: getSSMLResponse(speechOutput),
                directives : getDisplayRenderTemplateDirective(template),
                shouldEndSession: true
            });
            this.emit(':responseReady');
        },
        ':askWithDisplayRenderTemplate': function(speechOutput, repromptSpeech, template) {
            if(this.isOverridden()) {
                return;
            }
            this.handler.response = buildSpeechletResponse({
...
function getDisplayRenderTemplateDirective(template) {
    let directive = {
